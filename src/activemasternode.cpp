...
//
// Bootup the masternode, look for a 1000 XIOS input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in XIOS.conf */
...
{
    CXIOScoinAddress address(collateralAddress);
