...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: XIOS.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: XIOSd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeXIOSamount=<n> " + _("Keep N XIOS anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. XIOS is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. XIOS is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("XIOS version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "XIOS server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of XIOS") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart XIOS to complete") << "\n";
    ...
    nAnonymizeXIOSAmount = GetArg("-anonymizeXIOSamount", 0);
    if(nAnonymizeXIOSAmount > 999999) nAnonymizeXIOSAmount = 999999;
    if(nAnonymizeXIOSAmount < 2) nAnonymizeXIOSAmount = 2;
    ...
    LogPrintf("Anonymize XIOS Amount %d\n", nAnonymizeXIOSAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1XIOS+1000 == (.1XIOS+100)*10
       10XIOS+10000 == (1XIOS+1000)*10
    */
