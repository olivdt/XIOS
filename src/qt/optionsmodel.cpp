...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeXIOSAmount"))
        settings.setValue("nAnonymizeXIOSAmount", 1000);
    nAnonymizeXIOSAmount = settings.value("nAnonymizeXIOSAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeXIOSAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeXIOSAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeXIOSAmount:
            return QVariant(nAnonymizeXIOSAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeXIOSAmount:
            nAnonymizeXIOSAmount = value.toInt();
            settings.setValue("nAnonymizeXIOSAmount", nAnonymizeXIOSAmount);
            emit AnonymizeXIOSAmountChanged(nAnonymizeXIOSAmount);
            break;
        default:
            break;
        }
