...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XIOS
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XIOS
    // Mac: ~/Library/Application Support/XIOS
    // Unix: ~/.XIOS
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XIOS";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "XIOS.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "XIOSd.pid"));
